...
function main(params)
{
  ...
  G = gear; loop(0); // added by kariya1975@gmail.com for animation
  return gear;
}
...
// added by kariya1975@gmail.com for animation.
function render(obj) {
    self.postMessage({cmd:'rendered', result:obj.toCompactBinary()});
}
...
function loop(n) {
    if (n <= 180) {
        render(G.rotateZ(n)
            .union(G.setColor(0,1,0).rotateZ(-n).translate([16,0,0]))
            .union(G.setColor(0,0,1).rotateZ(-n).translate([-16,0,0]))
        );
        loop(n+9);
        //setTimeout(function () { loop(n + 9); }, 0);
    }
} 
